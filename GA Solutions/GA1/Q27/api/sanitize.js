// Vercel Serverless function wrapper for sanitize logic (CommonJS)
...
function trackRate(ip) {
  ...
  if (elapsed > RATE_LIMIT_WINDOW_MS) { entry.count = 1; entry.start = now(); } else { entry.count += 1; }
...
function redactPII(s) {
  ...
  out = out.replace(/\b([A-Z][a-z]+)\s+([A-Z][a-z]+)\b/g, (m) => {
    if (/\b(Street|St|Ave|Avenue|Mr|Mrs|Ms|Dr|Inc|LLC)\b/.test(m)) return m;
    redactions.names++; return '[REDACTED NAME]';
  });
...
module.exports = async function (req, res) {
  try {
    ...
    if (!trackRate(ip)) return res.status(429).json({ blocked: true, reason: 'rate limit exceeded', sanitizedOutput: '', confidence: 0.0 });
    if (req.method !== 'POST') return res.status(400).json({ blocked: true, reason: 'invalid method', sanitizedOutput: '', confidence: 0.0 });
    const body = req.body;
    if (!body || typeof body !== 'object') return res.status(400).json({ blocked: true, reason: 'invalid JSON body', sanitizedOutput: '', confidence: 0.0 });
    ...
    if (detectSqlInjection(stripped)) return res.status(200).json({ blocked: true, reason: 'SQL injection pattern detected', sanitizedOutput: '', confidence: 0.0 });
